...
async function getDocumentsWithPartialSQL(partialSQL) {
	const selectSQL = "SELECT d.document_id, d.cord_uid, d.pubmed_id, d.title, d.url, d.journal, d.publish_year, d.publish_month, d.publish_day, d.doi, d.is_preprint, d.altmetric_id, d.altmetric_score, d.altmetric_score_1day, d.altmetric_score_1week, d.altmetric_openaccess, d.altmetric_badgetype " + partialSQL
...
export async function getTableData(page_info) {
	const showNonResearch = 'show_nonresearch' in page_info && page_info['show_nonresearch']
	const dbCategoryName = 'altname' in page_info ? page_info.altname : page_info.name
	
	var partialSQL
	if (showNonResearch) {
		partialSQL = await getByCategory_partialSQL(dbCategoryName)
	} else {
		partialSQL = await getResearchByCategory_partialSQL(dbCategoryName)
	}
