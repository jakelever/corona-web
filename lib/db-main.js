...
async function getResearchPapers_partialSQL(topic) {
	...
	const metaanalysisID = await getEntityID('Meta-analysis','articletype')
	...
	AND (anno_articletype.entity_id = ${researchID} OR anno_articletype.entity_id = ${metaanalysisID})
	

	`
}

async function getPapersByArticleType_partialSQL(articletype) {
	const reviewID = await getEntityID(articletype,'articletype')
	...
	AND anno_articletype.document_id 
	AND anno_articletype.entity_id = ${reviewID}
...
export async function getTableData(pagename) {
	var partialSQL
	if (pagename == 'Reviews') {
		partialSQL = await(getPapersByArticleType_partialSQL('Review'))
	} else if (pagename == 'Comments & Editorials') {
		partialSQL = await(getPapersByArticleType_partialSQL('Comment/Editorial'))
	} else if (pagename == 'Updates') {
		partialSQL = await(getPapersByArticleType_partialSQL('Updates'))
	} else if (pagename == 'Book Chapters') {
		partialSQL = await(getPapersByArticleType_partialSQL('Book chapter'))
	} else if (pagename == 'News') {
		partialSQL = await(getPapersByArticleType_partialSQL('News'))
	} else if (pagename == 'Erratum') {
		partialSQL = await(getPapersByArticleType_partialSQL('Erratum'))
	} else if (pagename == 'Case Reports') {
		partialSQL = await(getPapersByArticleType_partialSQL('Case Reports'))
	} else {
...
export async function getChartDataByVirusInTopic(topic,entitytype,topLimit) {
	const topicOrArticleType = topic == 'Case Reports' ? 'articletype' : 'topic'
	...
	WHERE tmp1.document_id IN (
		...
		WHERE a.entity_id = e.entity_id 
		AND e.entitytype_id = et.entitytype_id 
		AND et.name = ${topicOrArticleType}
		AND e.name = ${topic}
	)
