...
export async function getChartDataByVirus_old(topic,entitytype) {
	var counts = await db.query(escape`
	...
	var countsByEntityOnly = []
	//var countsByEntityAndVirus = []
	var countsByEntityAndVirus = {'SARS-CoV-2':[],'MERS-CoV':[],'SARS-CoV':[]}
	counts.forEach(c => { 
		if (c['entity_name'] in countsByEntityOnly) {
			countsByEntityOnly[c['entity_name']] += c['count']
		} else {
			countsByEntityOnly[c['entity_name']] = c['count']
		}
	...
	const topEntities = Object.keys(countsByEntityOnly).sort(function(a, b) {
	  //return countsByEntityOnly[a] - countsByEntityOnly[b];
	  if (countsByEntityOnly[a] < countsByEntityOnly[b])
		  return 1
	  else if (countsByEntityOnly[a] > countsByEntityOnly[b])
		  return -1
	  else
		  return 0
	}).slice(0,30)
	...
	const viruses = counts.map(c => c['virus_name']).filter(onlyUnique)
	
	var countsByVirus = []
	viruses.forEach(v => {
		countsByVirus[v] = topEntities.map(e => {
			return (v in countsByEntityAndVirus && e in countsByEntityAndVirus[v]) ? countsByEntityAndVirus[v][e] : 0
		})
	})
	
	const finalData = {entities:topEntities, counts: countsByVirus}
	
	return finalData
}

export async function getChartDataByVirus(topic,entitytype) {
	...
	const counts = dbResponse.map(r => Object.assign({},r))
