...
async function getResearchPapers_partialSQL(topic) {
	...
	const metaanalysisID = await getEntityID('Meta-analysis','pubtype')
	...
	AND (anno_pubtype.entity_id = ${researchID} OR anno_pubtype.entity_id = ${metaanalysisID})
...
export async function getTableData(pagename) {
	...
	if (pagename == 'Reviews') {
		...
		partialSQL = await(getPapersByPubType_partialSQL('Book chapter'))
	} else {
