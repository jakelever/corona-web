...
export async function getTopicCountsByVirus() {
	var counts = await db.query(escape`
	...
	counts.forEach( row => {
		...
		if (! (topic in wide_data)) {
			wide_data[topic] = {'topic':topic,'MERS-CoV':0,'SARS-CoV':0,'SARS-CoV-2':0}
		}
		
		wide_data[topic][v] = count
	})
...
export async function getArticleTypeCountsByVirus() {
	var counts = await db.query(escape`

	SELECT e_articletype.name as articletype, e_virus.name as virus, COUNT(*) as count
	FROM annotations anno_articletype, entities e_articletype, entitytypes et_articletype, annotations anno_virus, entities e_virus, entitytypes et_virus
	WHERE anno_articletype.document_id = anno_virus.document_id
	AND anno_articletype.entity_id = e_articletype.entity_id
	AND e_articletype.entitytype_id = et_articletype.entitytype_id
	AND et_articletype.name = 'articletype'
	AND anno_virus.entity_id = e_virus.entity_id
	AND e_virus.entitytype_id = et_virus.entitytype_id
	AND et_virus.name = 'Virus'
	GROUP BY articletype, virus
	ORDER BY count
	`)
	...
	counts.forEach( row => {
		const v = row['virus']
		const articletype = row['articletype']
		const count = row['count']
		
		if (! (articletype in wide_data)) {
			wide_data[articletype] = {'articletype':articletype,'MERS-CoV':0,'SARS-CoV':0,'SARS-CoV-2':0}
		}
		
		wide_data[articletype][v] = count
	})
...
export async function getSummaryStatistics() {
	...
	FROM entities e, entitytypes et
	WHERE e.entitytype_id = et.entitytype_id
	AND et.name = 'articletype'

	`)
	
	const articletypeCount = result[0].count
